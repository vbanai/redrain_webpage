...
# # FastAPI endpoints: we should use request.app.state.redis_client (request-scoped, safe for multiple workers).

# # redis = request.app.state.redis_client
# # …it means:
# # “Take the app instance that is handling this HTTP request, and fetch the redis_client we stored in its state during startup.
# # request is the FastAPI Request object for the current incoming HTTP request.
# # request.app is not a variable you defined manually. It’s an attribute of the Request object provided by FastAPI.
# # FastApi automatically sets: request.app → the FastAPI instance that is handling this request
# # request.app points to fastapi_app internally, for this request.

# # Socket.IO handlers: we use fastapi_app.state.redis_client (global in-process reference, safe as long as each worker sets it up on startup).
# # If later scaling to multiple FastAPI workers, Redis connection must be initialized in each worker (which we already do in the lifespan/startup function)
...
# fastapi_app.state.minilm_model_encoder_for_clf_classifier = None
# fastapi_app.state.lr_classifier = None
